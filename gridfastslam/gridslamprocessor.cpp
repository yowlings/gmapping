...
GridSlamProcessor::GridSlamProcessor(const GridSlamProcessor& gsp)
  ...
  cerr << "Tree: normalizing, resetting and propagating weights within copy "
          "construction/cloneing ...";
...
GridSlamProcessor* GridSlamProcessor::clone() const {
  ...
  typedef std::map<autoptr<Array2D<PointAccumulator> >::reference* const, int>
      PointerMap;
  PointerMap pmap;
  ...
  cerr << __PRETTY_FUNCTION__ << ": Number of allocated chunks" << pmap.size()
       << endl;
  for (PointerMap::const_iterator it = pmap.begin(); it != pmap.end(); it++)
    assert(it->first->shares == (unsigned int)it->second);

  cerr << __PRETTY_FUNCTION__ << ": SUCCESS, the error is somewhere else"
       << endl;
#endif
...
void GridSlamProcessor::setMatchingParameters(
    double urange, double range, double sigma, int kernsize, double lopt,
    double aopt, int iterations, double likelihoodSigma, double likelihoodGain,
...
void GridSlamProcessor::setUpdateDistances(double linear, double angular,
  ...
  if (m_infoStream)
    m_infoStream << " -linearUpdate " << linear << " -angularUpdate " << angular
...
bool GridSlamProcessor::processScan(const RangeReading& reading,
  ...
  if (m_outputStream.is_open()) {
    m_outputStream << setiosflags(ios::fixed) << setprecision(6);
    m_outputStream << "ODOM ";
    m_outputStream << setiosflags(ios::fixed) << setprecision(3) << m_odoPose.x
                   << " " << m_odoPose.y << " ";
    m_outputStream << setiosflags(ios::fixed) << setprecision(6)
                   << m_odoPose.theta << " ";
  ...
  if (m_outputStream.is_open()) {
    ...
    for (ParticleVector::iterator it = m_particles.begin();
      ...
      m_outputStream << setiosflags(ios::fixed) << setprecision(3) << pose.x
                     << " " << pose.y << " ";
      m_outputStream << setiosflags(ios::fixed) << setprecision(6) << pose.theta
                     << " " << it->weight << " ";
    }
    m_outputStream << reading.getTime();
    m_outputStream << endl;
  ...
  if (m_linearDistance > m_distanceThresholdCheck) {
    cerr << "******************************************************************"
            "*****"
         << endl;
    cerr << "********** Error: m_distanceThresholdCheck overridden!!!! "
            "*************"
         << endl;
    ...
    cerr << "******************************************************************"
            "*****"
         << endl;
    cerr << "** The Odometry has a big jump here. This is probably a bug in "
            "the   **"
         << endl;
    cerr << "** odometry/laser input. We continue now, but the result is "
            "probably **"
         << endl;
    cerr << "** crap or can lead to a core dump since the map doesn't fit.... "
            "C&G **"
         << endl;
    cerr << "******************************************************************"
            "*****"
  ...
  if (!m_count || m_linearDistance >= m_linearThresholdDistance ||
    ...
    //		cerr  << "Tree: normalizing, resetting and propagating weights at the
    //end..." ;
    updateTreeWeights(false);
    //		cerr  << ".done!" <<endl;
  ...
  if (m_outputStream.is_open()) m_outputStream << flush;
  m_readingCount++;
  return processed;
}

std::ofstream& GridSlamProcessor::outputStream() { return m_outputStream; }

std::ostream& GridSlamProcessor::infoStream() { return m_infoStream; }
