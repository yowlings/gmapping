...
inline void GridSlamProcessor::scanMatch(const double* plainReading) {
    ...
    if (score > m_minimumScore) {
      ...
      if (m_infoStream) {
        ...
        m_infoStream << "lp:" << m_lastPartPose.x << " " << m_lastPartPose.y
                     << " " << m_lastPartPose.theta << std::endl;
        m_infoStream << "op:" << m_odoPose.x << " " << m_odoPose.y << " "
                     << m_odoPose.theta << std::endl;
      }
    }
  ...
  if (m_infoStream)
    m_infoStream << "Average Scan Matching Score="
                 << sumScore / m_particles.size() << std::endl;
}
...
inline void GridSlamProcessor::normalize() {
  ...
  m_neff = 0;
  for (std::vector<double>::iterator it = m_weights.begin();
       it != m_weights.end(); it++) {
    *it = *it / wcum;
    double w = *it;
    m_neff += w * w;
...
inline bool GridSlamProcessor::resample(const double* plainReading,
  ...
  if (m_neff < m_resampleThreshold * m_particles.size()) {
    ...
    if (m_outputStream.is_open()) {
      ...
      for (std::vector<unsigned int>::const_iterator it = m_indexes.begin();
           it != m_indexes.end(); it++) {
        m_outputStream << *it << " ";
      }
